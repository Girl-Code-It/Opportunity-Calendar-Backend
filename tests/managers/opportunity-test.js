...
describe('OpportunityManager', function () {
  ...
  describe('getOpportunities', function () {
    const stub = sinon.stub(Opportunity, 'find').returns({
      skip: async (skip) => fakedataArray.slice(skip == 0 ? 0 : skip - 1),
    });
    ...
    it('should have a previous page', async function () {
      ...
      expect(result.results.length).to.equal(3); // It is made intentionally wrong
    });
